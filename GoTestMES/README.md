...
- ✅ **8-byte Header 協定**: 支援 `[8-byte Length] + [JSON Payload]` 封包格式
...
├── main.go                 # Entry point
├── core/
│   ├── protocol.go         # 8-byte header packet handling
...
```
[8-byte ASCII Length] + [JSON Payload]
```

範例：
```
00000194{"type":"LINK","timestamp":"2025-12-01T10:30:00+08:00",...}
```
...
1. **8-byte Header**: 發送 JSON 時，務必計算 JSON byte 長度，並在前面加上 8 碼 ASCII（不足補 0）
